...
function GetNewsPosts($boardName, $section = null, $maxCount = 100) {
    if ($boardName != null && sql_query_into($result, "SELECT BoardId FROM ".FORUMS_BOARD_TABLE." WHERE UPPER(Name)=UPPER('".sql_escape($boardName)."');", 1)) {
        ...
        $news = array();
        if (sql_query_into($result, "SELECT * FROM ".FORUMS_POST_TABLE." WHERE IsThread=1 AND ParentId=$news_board_id AND NewsPost=1 ORDER BY PostDate DESC LIMIT $maxCount;", 1)) {
